<?php
    ...
    $user_details = $connectdb->prepare("SELECT * FROM users WHERE user_email = :user_email AND payment_status = 2");
        ...
        <tbody>
            ...
            <td>
                <?php
                    ...
                    $get_attendance->bindValue("pharmacist", $user->reg_number);
                ...
                <?php }else{ ?>
                <a style="background:green; color:#fff; padding:5px 10px;"href="javascript:void(0);" title="Mark present" onclick="attendEvent('<?php echo $row->event_id?>', '<?php echo $user->reg_number?>')"><i class="fas fa-check"></i> Attend</a>
                <?php }?>
            </td>
            </tr>
            <?php $n++; }?>
